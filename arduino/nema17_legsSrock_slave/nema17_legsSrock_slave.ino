...
int motorAccel = 2000 * microstepRes; //steps/second/second to accelerate
...
void loop(){
  ...
  if(wireState > 0) {
    ...
    else if(wireState == 203){
      stepper_one.move(-1 * (((((200 * 53.575757576) * 4) / 360) * 2) * 4.28));
      wireState = 0;       
    }    
    else if(wireState > 0 && wireState < 200){
      newAngle = (wireState - currentAngle);
      stepper_one.move(newAngle * (((((200 * 53.575757576) * 4) / 360) * 2) * 4.28)); //Move X times revolutions, according to gear reduction ratio. 
